...
class FrontendFiles extends Component
{
    ...
    async build()
    {
        ...
        try
        {
            ...
            apiURLFilesListing = gSystemConfig.configAPIURL + "/" + gSystemConfig.configRouteAPI + "/" + gSystemConfig.configRouteAPIFiles + "/" + this.idParentFiles + "/?fileType=3&apiKey=createSecretPassword";
                ...
                { arrFilesListing.length > 0 ?
                    <React.Fragment>
                        <h4 className="ss-frontend-files-title">
                            { SyncSystemNS.FunctionsGeneric.appLabelsGet(gSystemConfig.configLanguageFrontend.appLabels, "frontendFilesTitleMain") }
                        </h4>
